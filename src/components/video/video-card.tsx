...
interface VideoCardProps {
  ...
  showStats?: boolean; // For profile page - shows only views by default, likes/comments on hover
}

export function VideoCard({ video, orientation = 'horizontal', showStats = false }: VideoCardProps) {
      ...
      >
        <CardContent className="p-0 flex-1 flex flex-col">
            <div className="relative">
              ...
              {isVertical && showStats && (
                <>
                  {/* Default Overlay - Views Only (Instagram Profile Style) */}
                  <div className="absolute inset-0 bg-transparent group-hover:bg-transparent transition-all duration-300 rounded-b-lg pointer-events-none">
                    <div className="absolute bottom-2 left-2 text-white text-sm font-medium flex items-center gap-1.5 opacity-100 group-hover:opacity-0 transition-opacity duration-200">
                  ...
                  <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-center justify-center gap-6 rounded-b-lg">
                     <div className="flex items-center gap-2 text-white font-semibold">
                       <Heart className="w-6 h-6 fill-white" />
                       <span className="text-lg">{video.likes.toLocaleString()}</span>
                     </div>
                     <div className="flex items-center gap-2 text-white font-semibold">
                       <MessageCircle className="w-6 h-6 fill-white" />
                       <span className="text-lg">{video.commentsCount}</span>
                     </div>
                  </div>
                </>
              )}
